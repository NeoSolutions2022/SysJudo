...
public static class ModelBuilderExtension
{
    ...
    {
        var entidades = modelBuilder.GetEntities<ITenant>();

        var props = entidades
            .SelectMany(c => c.GetProperties().Where(p => p.ClrType == typeof(int))).ToList();

        foreach (var prop in props.Where(c => c.Name == "ClienteId"))
        {
            prop.IsNullable = false;
        }
    }
